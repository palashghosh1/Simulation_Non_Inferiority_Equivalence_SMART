  ...
  # this function will generate the data for non-inferiority and equivalence SMART:
  
  
  # calculate the sample size:
  cal_N <- function(sigma, theta, delta, g1, g2, alpha, beta, distinct_path, mu.AA, mu.AC, mu.AD, mu.BB, mu.BC, mu.BD,
                    mu.max, mu.min, use.Ext.Var){
...
func_repeat <- function(i, N, sigma, theta, g1, g2, alpha, beta, z_alpha, n1, n2,
  ...
  # Generate the primary variable Y:
  
  # dat <- dat%>%group_by(Trt.1, R, Trt.2)%>%
  #   mutate( Y = round(rnorm(n(), mu.y, sigma.star), 2) )

  # sigma.T1.T2 for robustness test:
  
  dat <- dat%>%group_by(Trt.1, R, Trt.2)%>%
    mutate( sigma.T1.T2 =  round(runif(1, max(0.5, sigma-1), sigma + 1), 2),
            Y = round(rnorm(n(), mu.y, sigma.T1.T2), 2) )
